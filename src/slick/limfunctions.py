...
mu_atom = (
    1.4 / 0.6
)  # atomic weight (mean mass per particle, equivalent to gmc.mu in despotic for xH2 = 0.5 and xHe = 0.1)
...
def densityProfile(mass, size, NZONES=25, ProfileType="Plummer"):
    ...
    if ProfileType == "Plummer":
        Rp = size / 10
        density = (
            3 * mass / (4.0 * np.pi * Rp**3) * (1.0 + radii**2 / Rp**2) ** (-5.0 / 2)
        )
        density = ((density / constants.m_p).cgs)[::-1]
    elif ProfileType == "Powerlaw":
        # Powerlaws
        alpha = 2
        n0 = mass / (4.0 * np.pi * size**2 * size ** (3.0 - alpha) / (3.0 - alpha))
        print("n0: " + str(n0))
        ...
        # Logotropic
        density = (mass / (4.0 / 3 * np.pi * size**3) / constants.m_p).cgs
        nHext = 2.0 / 3 * density
...
def compute_temp_dens(Mcloud, Rcloud, Tg, Td, NZONES):
    density, dR = densityProfile(
        Mcloud, Rcloud, NZONES, ProfileType="Powerlaw"
    )  # density is actually number density
    ...
    for i in range(NZONES - 1, -1, -1):
        volume_list.append(
            4 / 3 * np.pi * ((((i + 1) * dR[0].value) ** 3) - ((i * dR[0].value) ** 3))
        )
...
def submm_luminosity(INPUT, NZONES=25, ProfileType="Powerlaw", noClump=False):
    Mcloud, Rcloud, Metallicity, RadField, redshift, DMR, CR = (
        INPUT[0],
        INPUT[1],
        INPUT[2],
        INPUT[3],
    ...
    Pressure = 1.0e4 * (Rcloud.value / ((Mcloud.value / 290) ** 0.5)) ** (-4)  # P/kb
    ...
    if ProfileType != "Fixed":
        density, dR = densityProfile(
            Mcloud, Rcloud, NZONES, ProfileType=ProfileType
        )  # density is actually number density
        density /= mu_atom  # atomic weight correction
        ...
        gmc._colDen[1:] = (
            column_density_cgs.value
        )  # For some reason gmc._colDen gets changed in the previous line... Have to reset
    else:
        # a fixed average density
        density = (Mcloud / (4.0 / 3 * np.pi * Rcloud**3) / constants.m_p).cgs
        density /= mu_atom  # atomic weight correction
        column_density_cgs = density * Rcloud.to(u.cm)

        gmc = zonedcloud(
            colDen=np.linspace(
                column_density_cgs.value / NZONES, column_density_cgs.value, NZONES
            )
        )
        gmc._colDen = np.linspace(
            column_density_cgs.value / NZONES, column_density_cgs.value, NZONES + 1
        )
        gmc.colDen = gmc._colDen[1:] - (gmc._colDen[1:] - gmc._colDen[:-1]) / 2
    ...
    gmc.addEmitter("c+", 1.0e-100 * Metallicity)
    gmc.addEmitter("c", 1.6e-4 * Metallicity)
    gmc.addEmitter("o", 3.2e-4 * Metallicity)
    gmc.addEmitter("co", 1.0e-100 * Metallicity)

    gmc.TCMB = 2.73 * (1.0 + redshift)
    gmc.rad.TCMB = 2.73 * (1.0 + redshift)
    ...
    converged = gmc.setChemEqForSlick(
        network=GOW,
        evolveTemp="iterateDust",
        verbose=True,
        info={
            "xC": 1.6e-4 * Metallicity,
            "xO": 3.2e-4 * Metallicity,
            "xSi": 1.7e-6 * Metallicity,
        },
        tol=1.0e-3,
        maxTime=1e20,
    )
    ...
    for nz in range(NZONES):
        ...
        gmc.comp[nz].computeDerived(
            density[nz].value
        )  # line added to update mu and muH values

    # THe H2 fraction of the cloud is the fraction of H nuclei bound as H2. fH2 = 2N_H2/(2N_H2+N_HI+N_H+(+etc. H based species)).
    fH2 = np.sum(gmc.mass() * (2.0 * gmc_xH2_zones)) / np.sum(
        gmc.mass()
    )  # Note that mass in zonedcloud is already only the hydrogen atoms
    ...
    # CII
    H2_rates.append(
        gmc.dEdt(fixedLevPop=False, noClump=noClump)
    )  # get the cooling rates: note, I've already calculated the level populations before using LineLum, so take off the fixedlevpop if you haven't done that
    H2_LambdaCp = np.array(
        [r["LambdaLine"]["c+"] for r in H2_rates]
    )  # just pull out the line cooling rate for C+
    H2_lcii = (
        (H2_LambdaCp * H2_n_h_nuclei).decompose() * u.erg / u.s
    )  # LambdaCp was the cooling rate per h nucleon.   so i multiply by H2_n_h_nuclei which is my number of H per cloud, then by the number of the clouds in the galaxy

    if np.isnan(H2_lcii):
        H2_lcii = 0.0 * u.erg / u.s
    ...
    gas_temp, dust_temp, n_dens, col_dens = compute_temp_dens(
        Mcloud, Rcloud, gmc.Tg, gmc.Td, NZONES
    )

    Mol_gas = (fH2 * H2_n_h_nuclei * constants.m_p.to("g")).to(
        "Msun"
    )  # molecular gas considering only H2 mass (no helium) in units of Msun. This is later summed to get total Mol_gas of a galaxy

    CO10_Lsun = CO10.value / 3.826e33  # converting ergs/s to Lsun units
    CO10_areal_TB = (
        CO10_Lsun * 1e11 / (3 * (115.271203**3))
    )  # using Carilli and Walter 2013's equation on pg 9 to convert source luminosity in Lsun to areal integrated source brightness temperature in units of K km/s pc**2. This is later summed to get total CO10_areal_TB of the galaxy. 115.271203 GHz is the rest frame frequency of CO10 emission.
...
def creating_table(cloud_list, df_basic, output_dir, n_zones):
    ...
    for c in cloud_list:
        ...
        if RadField == 0 or RadField == -99:
            RadField = round(
                np.interp(redshift, np.array(UV_df["z"]), np.array(UV_df["UV"])), 6
            )
            ...
            df_aux = pd.DataFrame(
                {
                    ...
                    "col_dens": out[40],
                    "Mol_gas": out[41],
                    "CO10_areal_TB": out[42],
                    "Time": time,
                    "NZONES": n_zones,
                    "Zones_Converged": np.sum(out[43]),
                },
                index=[0],
            )
