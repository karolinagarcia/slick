...
def table_for_plots(df,df_basic):
    ...
    j = {'g_SFR': 'first', 'g_Redshift':'first', 'g_Mass_Gas': 'first', 'g_Mass_H2': 'first',
     'g_Radius': 'first', 'g_Metallicity': 'first'}
    df_gals_aux1 = df_basic.groupby(['Galaxy_ID'], as_index=False).agg(j)

    f = {'H2_lcii': 'sum', 'CO10': 'sum', 'CO21': 'sum', 'CO32': 'sum', 'CO43': 'sum',
        'CO54': 'sum', 'CO10_intTB': 'sum', 'CO21_intTB': 'mean', 'CO32_intTB': 'mean', 'CO43_intTB': 'mean',
        'CO54_intTB': 'mean', 'CI10': 'sum', 'CI21': 'sum', 'CO65': 'sum', 'CO76': 'sum', 'CO87': 'sum',
        'CO98': 'sum', 'CO65_intTB': 'mean', 'CO76_intTB': 'mean', 'CO87_intTB': 'mean', 'CO98_intTB': 'mean',
        'OI1': 'sum', 'OI2': 'sum','OI3': 'sum', 'fH2': 'sum'}
...
def train_model(df,df_basic,line):
    ...
    if type(df_basic['g_Mass_Gas'].iloc[0]) != np.float64:
        df_basic['g_Mass_Gas'] = df_basic['g_Mass_Gas'].map(lambda x: x.rstrip(' Msun')).astype(float)
        df_basic['g_Mass_H2'] = df_basic['g_Mass_H2'].map(lambda x: x.rstrip(' Msun')).astype(float)
        df_basic['g_Radius'] = df_basic['g_Radius'].map(lambda x: x.rstrip(' pc')).astype(float)
        df_basic['g_Metallicity'] = df_basic['g_Metallicity'].map(lambda x: x.rstrip(' dimensionless')).astype(float)

    input_cols = ['g_SFR', 'g_Mass_Gas', 'g_Radius', 'g_Metallicity']
    output_cols = ['H2_lcii','CO10','CO21','CO32','CO43','CO54','CO10_intTB','CI10','CI21','fH2']
...
def predict(df_basic,line,model):
    ...
    if type(df_basic_final['g_Mass_Gas'].iloc[0]) != np.float64:
        df_basic_final['g_Mass_Gas'] = df_basic_final['g_Mass_Gas'].map(lambda x: x.rstrip(' Msun')).astype(float)
        df_basic_final['g_Mass_H2'] = df_basic_final['g_Mass_H2'].map(lambda x: x.rstrip(' Msun')).astype(float)
        df_basic_final['g_Radius'] = df_basic_final['g_Radius'].map(lambda x: x.rstrip(' pc')).astype(float)
        df_basic_final['g_Metallicity'] = df_basic_final['g_Metallicity'].map(lambda x: x.rstrip(' dimensionless')).astype(float)
